...
export function Navbar() {
  ...
  return (
    <nav
      ...
      <div className="max-w-[1280px] mx-auto px-6 flex items-center justify-between h-[74px]">
        ...
        <Link href="/" className="flex items-center gap-3 no-underline">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{ background:'linear-gradient(135deg,var(--divine-purple),var(--deep-indigo))', boxShadow:'0 4px 16px rgba(107,75,161,0.5)' }}>
            {/* Replace <img> with <Image> once logo file exists at /public/logo/YLF Logo.png */}
            <svg width="26" height="26" viewBox="0 0 28 28" fill="none">
              <path d="M14 2L14 26M5 11L23 11" stroke="#f4a535" strokeWidth="2.5" strokeLinecap="round"/>
              <path d="M14 2C14 2 9 7 9 12C9 15.31 11.24 18 14 18C16.76 18 19 15.31 19 12C19 7 14 2 14 2Z" fill="rgba(244,165,53,0.2)" stroke="#f4a535" strokeWidth="1.5"/>
            </svg>
          </div>
          <div className="flex flex-col leading-none">
            <span className="font-serif font-bold text-[1.05rem] text-white">Yeshua's Love Family</span>
            <span className="text-[0.6rem] tracking-[0.15em] uppercase mt-0.5"
              style={{ color:'var(--holy-gold)' }}>YLF · Jamshedpur</span>
          </div>
        </Link>
          ...
          {NAV_LINKS.map(({ href, label }) => {
            ...
            return (
              <li key={href}>
                <Link
                  href={href}
                  className="relative px-3.5 py-2 text-[0.75rem] font-medium tracking-wider uppercase rounded-lg transition-colors duration-200 no-underline group"
                  style={{ color: active ? 'var(--holy-gold)' : 'rgba(255,255,255,0.82)' }}>
                  {label}
                  <span
                    className="absolute bottom-0 left-0 right-0 h-[2px] rounded-sm transition-all duration-300"
                    style={{
        ...
        <button
          ...
          {[0,1,2].map(i => (
            <span key={i} className="block w-6 h-[2px] rounded-sm bg-white transition-all duration-300"
              style={{
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="lg:hidden flex flex-col px-6 pb-5 gap-1"
