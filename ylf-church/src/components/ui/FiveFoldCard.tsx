...
export function FiveFoldCard({ ministry, index }: Props) {
  return (
    <div
      className="h-full flex flex-col rounded-[18px] p-6 text-center border border-white/10 transition-all duration-300 hover:-translate-y-1 cursor-default relative overflow-hidden"
      style={{
    ...
    >
      ...
      {/* Role */}
      <p
        className="text-xs font-bold tracking-[.12em] uppercase mb-2"
      ...
      {/* Title */}
      <h3 className="font-serif font-bold text-white mb-3 text-base">
        {ministry.title}
      </h3>
      ...
      {/* Scripture */}
      {ministry.scripture && (
        <div className="mt-5 pt-4 border-t border-white/10 h-[58px] flex items-center justify-center">
          <p
            className="text-[0.72rem] italic text-center"
            style={{ color: 'rgba(244,165,53,0.6)' }}
          >
            {ministry.scripture}
          </p>
        </div>
